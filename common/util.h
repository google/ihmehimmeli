...
#ifndef IHMEHIMMELI_COMMON_UTIL_H_
#define IHMEHIMMELI_COMMON_UTIL_H_
...
namespace detail {
template <typename T>
struct ZeroOutVector;
template <typename T>
struct ZeroOutVector<std::vector<T>> {
  static void Run(std::vector<T> *v) {
    for (T &x : *v) ZeroOutVector<T>::Run(&x);
  }
};
template <>
struct ZeroOutVector<float> {
  static void Run(float *v) { *v = 0; }
};
template <>
struct ZeroOutVector<double> {
  static void Run(double *v) { *v = 0; }
};
template <>
struct ZeroOutVector<size_t> {
  static void Run(size_t *v) { *v = 0; }
};
}  // namespace detail

template <typename T>
void ZeroOutVector(std::vector<T> *v) {
  detail::ZeroOutVector<std::vector<T>>::Run(v);
}
...
#endif  // IHMEHIMMELI_COMMON_UTIL_H_
