...
void Tempcoder::ComputeGradients(
    const VectorXd& final_outputs, const VectorXXd& activations,
  ...
  // Align outputs and targets. Currently just align outputs to t=1.
  // The alternative is aligning them to
  // std::min_element(final_outputs.begin(), final_outputs.end()), which will
  // require an extra derivative.
  double latency = use_mse_ ? 1.0 : 0.0;
...
const Prediction Tempcoder::FeedforwardAndOptionallyAccumulateUpdates(
  ...
  ComputeGradients(final_outputs, activations, targets, causal_sets, a, b, w,
...
double Tempcoder::ComputeLossWithPenalty(const VectorXd& outputs,
  ...
  if (use_mse_) {
    double latency = 1;  //*std::min_element(outputs.begin(), outputs.end());
    for (int i = 0; i < targets.size(); ++i) {
      double target = targets[i] == kNoSpike ? 1.0 : targets[i];
      double output = outputs[i] == kNoSpike ? 1.0 : outputs[i] - latency;
