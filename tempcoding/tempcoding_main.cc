...
ABSL_FLAG(std::string, problem, "xor",
          "problem (one of: and, or, xor, circle, iris, mnist, mnist2, "
          "mnist_ae, cifar10)");
...
void Main(int argc, char** argv) {
  ...
  for (int run = 0; run < n_runs; ++run) {
    ...
    tempcoder.use_mse() = problem.problem_type() == ProblemType::MNIST_AE;
    ...
    test_options.compute_train_accuracy = true;
